...
def handle_axioms(operators, axioms, goals, layer_strategy):
    if CLUSTER_NEGATIVE:
        normal_axioms, negative_axioms = axioms, []
    else:
        normal_axioms, negative_axioms = [], []
        for axiom in axioms:
            if axiom.effect.predicate.endswith(NEGATIVE_SUFFIX):
                negative_axioms.append(axiom)
            else:
                normal_axioms.append(axiom)

    clusters = compute_clusters(normal_axioms, goals, operators)
    axiom_layers = compute_axiom_layers(clusters, layer_strategy)
    for axiom in negative_axioms:
        axiom_layers[axiom.effect] = 0
    ...
    normal_axioms = get_axioms(clusters)
    axioms = negative_axioms + normal_axioms
    if DEBUG:
